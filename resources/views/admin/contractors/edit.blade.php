...
@section('content')
<div class="container">
    <h1 class="mb-4">Edit Contractor</h1>
    <form action="{{ route('contractors.update', $contractor->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="type" class="form-select @error('type') is-invalid @enderror">
                <option value="individual" {{ $contractor->type == 'individual' ? 'selected' : '' }}>Individual</option>
                <option value="legal" {{ $contractor->type == 'legal' ? 'selected' : '' }}>Legal</option>
            </select>
            @error('type')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        ...
        <div class="mb-3">
            ...
            <select name="role" class="form-select @error('role') is-invalid @enderror">
                <option value="customer" {{ $contractor->role == 'customer' ? 'selected' : '' }}>Customer</option>
                <option value="performer" {{ $contractor->role == 'performer' ? 'selected' : '' }}>Performer</option>
            </select>
            @error('role')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
